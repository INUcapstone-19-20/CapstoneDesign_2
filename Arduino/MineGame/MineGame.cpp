...
MineGame::MineGame(Player p, uint8_t y_dim, uint8_t x_dim)
{
    this->mode = "single";
    this->_y_dim = y_dim;
    this->_x_dim = x_dim;
...
MineGame::MineGame(Player pR, Player pB, uint8_t y_dim, uint8_t x_dim);
{
    this->mode = "battle";
    this->_y_dim = y_dim;
    this->_x_dim = x_dim;
...
void MineGame::setPlayer(Player p) 
{
    // set player
    p.ID = "single";
    p.color[0] = SINGLE1;
...
void MineGame::setPlayer(Player pR, Player pB)
{
    // set Player1
    pR.ID = "Red";
    pR.color[0] = RED1;
...
void MineGame::setPlayerColorArray(Player p, uint32_t *arr) 
{
    uint8_t rows = this->_y_dim, cols = this->_x_dim;
    
    // number -> x, y
    uint8_t mine_x = p.mine % cols;
    uint8_t mine_y = p.mine / cols;
    for (uint8_t y = 0; y < rows; y++) {
        for (uint8_t x = 0; x < cols; x++) {
            double d = distance(mine_x, mine_y, x, y);
            uint32_t keynum = y * cols + x;
            if (d <= DISTANCE1) 
...
void MineGame::initPlayer(Player p)
{
    p.ID = "";
    for(uint8_t i=0; i<COLORLEVEL; i++) p.color[i] = 0x000000;
    p.mine = 0;
}
