...
(싱글모드)
1. 탐색횟수, 시간초 얼마 안남았을 때 아두이노로 얼마 안남았다는 시리얼 통신 보내기
2. 탐색횟수, 시간초 얼마 안남았을 때 Danger! 팝업창
3. ~~최종 결과에서 소요시간 01:13 등의 형식을 바꾸기~~<b> </b> <code>완료</code>
4. 첫 버튼 클릭하면 제한시간이 감소하기 시작하도록(통신 필요?)

(배틀모드)
1. '지뢰가 터졌습니다' 화면 나오고 클릭 없이 2초 후에 '빨강(파랑) 플레이어 승리' 화면 나오도록
2. 재시작 or 처음으로 눌렀을 때 버튼 전체 off 되도록 (현재는 '지뢰가 터졌습니다' 화면 누르면 off)
3. 처음에만 '턴 횟수 정하기' 화면 나오고 게임 진행 중에는 중간 중간 '턴 횟수 정하기' 하면 안나오도록 하기
4. '빨강(파랑)플레이어 승리' 화면이 너무 밋밋함
5. 모든 선택만 [     ] 넣고 나머지는 다 빼기
6. pass 나오는 확률 낮추기?

### [communication.py]
1. 싱글모드에서 탐색횟수 얼마 안남았을 때 아두이노로 얼마 안남았다는 시리얼 통신 보내기 
2. 싱글모드에서 시간초 얼마 안남았을 때 아두이노로 얼마 안남았다는 시리얼 통신 보내기
...
(싱글모드)
1. 시간초, 탐색횟수 끝나면 끝났다는 이펙트 넣기(기존 지뢰 터지는 이펙트!)
2. 지뢰를 찾은 경우, 지뢰 부분은 여러 색 반복이 아니라 초록색 내에서 반복으로
   
   나머지 LED부분들은 모두 같은 초록색으로 점차적 확산
   
   or 글자로 LOOSE FAIL 등등
   
   or 선글라스 쓴 이모티콘, 웃는 이모티콘
3. 시간초와 탐색횟수가 거의 다 되면 LED 전체가 깜빡 깜빡 거리도록(점점 더 깜빡임 빨라지도록?)

(배틀모드)
1. 재시작 or 처음으로 눌렀을 때 버튼 전체 off 되도록 (현재는 '지뢰가 터졌습니다' 화면 누르면 off)
