![minefind_poster](https://user-images.githubusercontent.com/48705652/104692809-ede05d80-574b-11eb-82ed-996515b91be3.jpg) <br><br>
***

## 시연영상
[![Watch the video](https://user-images.githubusercontent.com/48705652/104693999-e3bf5e80-574d-11eb-8590-20882a256b9a.png)](https://youtu.be/heKlfEPI5kA) <br>
...
1. ~~게임실행 파일 바탕화면으로 이동~~<b> </b> <code>완료</code>

(싱글모드)
1. ~~탐색횟수, 시간초 얼마 안남았을 때 아두이노로 얼마 안남았다는 시리얼 통신 보내기~~<b> </b> <code>완료</code>
2. ~~탐색횟수, 시간초 초과했을 때 아두이노로 시간초과 시리얼 통신 보내기~~<b> </b> <code>완료</code>
3. ~~탐색횟수, 시간초 얼마 안남았을 때 Danger! 팝업창~~<b> </b> <code>완료</code>
...
4. ~~'빨강(파랑)플레이어 승리' 화면이 너무 밋밋함~~<b> </b> <code>완료</code>
5. ~~모든 선택만 [     ] 넣고 나머지는 다 빼기~~<b> </b> <code>완료</code>
6. ~~pass 나오는 확률 낮추기?~~<b> </b> <code>완료</code>

### [communication.py]
1. ~~싱글모드에서 탐색횟수 얼마 안남았을 때 아두이노로 얼마 안남았다는 시리얼 통신 보내기~~<b> </b> <code>완료</code>
2. ~~싱글모드에서 시간초 얼마 안남았을 때 아두이노로 얼마 안남았다는 시리얼 통신 보내기~~<b> </b> <code>완료</code>
3. ~~경고전달함수에 delay 매개변수로 전달~~<b> </b> <code>완료</code>
...
~~1. 시작 애니메이션 변경 :~~
  ...
  ~~→   (변경) 선글라스 이모티콘~~<b> </b> <code>완료</code>
  
2. ~~싱글모드 결과에서 '처음으로' 누르고 배틀모드 들어갔는데 LED 하나 켜져있는 버그? (주사위 횟수도 하나 감소됨)~~<b> </b> <code>완료</code>

3. ~~턴 횟수 1 남았을 때 버튼 2개 따닥 누르면 버그 발생 → 다음 턴부터 락도 안걸림~~<b> </b> <code>완료</code>

(싱글모드)
1. ~~시간초, 탐색횟수 끝나면 끝났다는 이펙트 넣기(기존 지뢰 터지는 이펙트! and 글자로 LOSE, FAIL 등등)~~<b> </b> <code>완료</code>
2. ~~지뢰를 찾은 경우, 지뢰 부분은 여러 색 반복이 아니라 초록색 내에서 반복으로~~<b> </b> <code>완료</code>
   
3. ~~지뢰 찾았을 때 나머지 LED부분들 배틀모드처럼 거리함수 확산~~<b> </b> <code>완료</code>
   
4. ~~실패했을 때 눈 X자 이모티콘~~<b> </b> <code>완료</code>

1. ~~시간초와 탐색횟수가 거의 다 되면 LED 전체가 깜빡 깜빡 거리도록(점점 더 깜빡임 빨라지도록?)<b> </b> <code>통신만 추가</code>→ 깜빡이는 부분 코드 추가 필요!~~<b> </b> <code>완료</code>

(배틀모드)
1. ~~재시작 or 처음으로 눌렀을 때 버튼 전체 off 되도록 (현재는 '지뢰가 터졌습니다' 화면 누르면 off)~~<b> </b> <code>완료</code>
2. ~~지뢰색 여러 색 반복이 아니라 빨강플레이어는 빨간색 내에서 반복, 파랑플레이어는 파란색 내에서 반복으로 수정~~<b> </b> <code>완료</code>
3. ~~지뢰 찾았을 때 터진 지뢰 말고 이긴 플레이어 지뢰도 표시~~<b> </b> <code>완료</code>
