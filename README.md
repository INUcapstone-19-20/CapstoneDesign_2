...
1. 재시작, 처음으로 눌렀을 때 가끔씩 오류 발생 (LED 안꺼짐, 지뢰 초기화 X)
-> 재시작 천천히 누르면 가능? 빨리 눌러도 가능하도록!
1. 게임 끝나고 '지뢰 터짐 LCD' 누르고나서 재시작 누르기 전에 버튼 눌리는 문제 해결
1. 전원 켜질 때도 아두이노 애니메이션 나오고, 라즈베리파이 켜질 때도 애니메이션 나옴
-> 하나만 되도록!

~~12x12 전력 문제 :
전압은 맞는데 전류가 부족? -> 결론적으로 전력이 부족~~ <code>완료</code>
