...
3. 탐색횟수, 시간초 얼마 안남았을 때 Danger! 팝업창<b> </b> <code>리소스 추가 완료</code> -> 화면 구성해주세요
...
2. 싱글모드 결과에서 '처음으로' 누르고 배틀모드 들어갔는데 LED 하나 켜져있는 버그? (주사위 횟수도 하나 감소됨)

3. 턴 횟수 1 남았을 때 버튼 2개 따닥 누르면 버그 발생 → 다음 턴부터 락도 안걸림

(싱글모드)
1. ~~시간초, 탐색횟수 끝나면 끝났다는 이펙트 넣기(기존 지뢰 터지는 이펙트! and 글자로 LOSE, FAIL 등등)<b> </b> <code>통신만 추가</code> → 지뢰 터지는 부분 코드 추가 필요!~~<b> </b> <code>완료</code>
