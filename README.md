...
4. '빨강(파랑)플레이어 승리' 화면이 너무 밋밋함<b> </b> <code>리소스 추가 완료</code> -> 화면 구성해주세요
...
3. ~~경고전달함수에 delay 매개변수로 전달~~<b> </b> <code>수정</code>
...
(싱글모드)
1. ~~시간초, 탐색횟수 끝나면 끝났다는 이펙트 넣기(기존 지뢰 터지는 이펙트! and 글자로 LOSE, FAIL 등등)<b> </b> <code>통신만 추가</code> → 지뢰 터지는 부분 코드 추가 필요!~~<b> </b> <code>완료(테스트 필요)</code>
2. ~~지뢰를 찾은 경우, 지뢰 부분은 여러 색 반복이 아니라 초록색 내에서 반복으로~~<b> </b> <code>완료(테스트 필요)</code>
   
3. ~~지뢰 찾았을 때 나머지 LED부분들 배틀모드처럼 거리함수 확산~~<b> </b> <code>완료</code>
   
~~4  실패했을 때 눈 X자 이모티콘~~<b> </b> <code>완료</code>

5. ~~시간초와 탐색횟수가 거의 다 되면 LED 전체가 깜빡 깜빡 거리도록(점점 더 깜빡임 빨라지도록?)<b> </b> <code>통신만 추가</code>→ 깜빡이는 부분 코드 추가 필요!~~<b> </b> <code>완료(테스트 필요)</code>

(배틀모드)
1. ~~재시작 or 처음으로 눌렀을 때 버튼 전체 off 되도록 (현재는 '지뢰가 터졌습니다' 화면 누르면 off)~~<b> </b> <code>완료</code>
2. ~~지뢰색 여러 색 반복이 아니라 빨강플레이어는 빨간색 내에서 반복, 파랑플레이어는 파란색 내에서 반복으로 수정~~<b> </b> <code>완료(테스트 필요)</code>
3. ~~지뢰 찾았을 때 터진 지뢰 말고 이긴 플레이어 지뢰도 표시~~<b> </b> <code>완료(테스트 필요)</code>
